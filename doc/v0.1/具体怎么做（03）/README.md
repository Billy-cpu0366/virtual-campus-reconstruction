...
节点总账是节点状态的唯一所有者。详细文档可以复制当前状态用于检查，但状态变化必须先更新 [[../整体怎么运作（02）/功能总目录（节点清单）]]，再同步主定义文档。
...
不满足条件的节点继续保留在 [[../整体怎么运作（02）/功能总目录（节点清单）]]、候选关注点去向表或 [[../还不清楚什么（05）/还缺哪些答案（未知问题队列）]]。创建文档不等于允许实现；代码仍需独立 Human Gate。

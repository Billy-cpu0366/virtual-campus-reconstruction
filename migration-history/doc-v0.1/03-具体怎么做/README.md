...
当前level-1任务卡：[[系统/图层与遮挡：最小视觉补证任务卡（SYS-LAYER）]]。它已在世界/图层设计工作项关闭后激活，只允许5个公开原站场景和白名单输出。

治理类任务卡：[[总文档迁移：本轮怎样合并文档入口（治理任务卡）]]、[[旧总文档清理：怎样保留跳转并推送（治理任务卡）]]。治理任务没有对应游戏功能节点时使用`node-refs: not-applicable`，只允许任务卡白名单内的结构和文档操作，不得借此写正式代码。

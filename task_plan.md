...
5. 新建 `整体怎么运作（02）/现有复刻实现全景.md` 与 `怎么验证与还差什么（04）/现有复刻行为基线.md`，更新证据追踪、发现和进度记录；
...
5. 建立 [[doc/v0.1/具体怎么做（03）/系统/SYS-CHUNK-地图分块-逆向工作稿]]，更新证据追踪和验证计划；在 Human 审查前不登记为唯一详细主定义；
...
当前输入：[[doc/v0.1/具体怎么做（03）/系统/SYS-CHUNK-地图分块#SYS-CHUNK]] 与 [[doc/v0.1/怎么验证与还差什么（04）/SYS-CHUNK-验证计划]]。

...
- **allowed**：修改 `src/layer/`、必要的 `src/world/` contract/hooks、`game/PhaserWorldRenderer.ts`、必要 `game/CampusScene.ts`、对应测试和验证脚本，实现 visual/roof/marker/footsteps 的 apply/remove、失败边界和 particles3 未消费诊断。
- **forbidden**：不修改 `sample/` 或旧 Phaser；不实现动态抗议人群、cars/NPC、特殊13层生命周期、请求取消、性能、完整 teardown 或其他玩法线。
- **status**：当前进入 `implementation / implementation`；尚未写正式代码，下一步先落盘状态并开始实现。

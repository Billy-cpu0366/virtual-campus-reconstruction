...
- 路线第4步已执行：46份旧`doc/v0.1` Markdown按原结构移动到`migration-history/doc-v0.1/`，逐文件哈希一致；当前视觉任务卡原位保留。旧治理检查器在历史区保存同哈希快照，`sample/tools`原文件未动；`task_plan.workflow-ref`已切到现操作手册。等待步骤检查和独立提交。
